"""
流程03：训练数据预处理主脚本
功能：从原始CSV/NPY数据中提取、变换和可视化训练数据

依赖：流程02必须先执行，以生成 measures_ID_auto.csv 文件

输入：
    - C:/abaqus_gen_data/ 目录下的原始数据文件
    - 02_inp_to_vtu_output/measures_ID_auto.csv（由流程02生成）
输出：
    - 03_preprocess_training_data_output/preprocessed_data_raw.npz (原始数据)
    - 03_preprocess_training_data_output/preprocessed_data.npz (变换后数据，可选)
    - 03_preprocess_training_data_output/transforms.joblib (变换器，可选)
    - 03_preprocess_training_data_output/*.png (可视化图像)
    - 03_preprocess_training_data_output/*.csv (图像对应的数据)
"""
...
SOURCE_DATA_DIR = r'C:\abaqus_gen_data'                     # 原始数据源目录
OUTPUT_DIR = os.path.join(WORKSPACE_DIR, 'script', '03_preprocess_training_data_output')  # 输出目录
...
PREPROCESS_MAX_FOLDERS = 2000                               # 最大处理文件夹数
...
def plot_grid_scatter_density(
    ...
    save_figure(fig, name=name, data=data_to_csv)
...
def _prepare_output_workspace(remove_raw: bool) -> None:
    """清理输出目录"""
    print("[清理] 正在清理预处理输出目录...")
...
print("流程03：训练数据预处理")
print("=" * 60)
print(f"数据源目录: {SOURCE_DATA_DIR}")
print(f"输出目录: {OUTPUT_DIR}")
print(f"测点ID文件: {MEASURES_ID_CSV}")
print(f"最大处理文件夹数: {PREPROCESS_MAX_FOLDERS}")
...
# 输出路径
processed_data_path_npz = os.path.join(OUTPUT_DIR, "preprocessed_data.npz")
raw_data_path_npz = os.path.join(OUTPUT_DIR, "preprocessed_data_raw.npz")
transforms_path = os.path.join(OUTPUT_DIR, "transforms.joblib")
...
if DO_COLLECT:
    ...
    if not os.path.isdir(SOURCE_DATA_DIR):
        raise FileNotFoundError(f"错误: 数据源目录不存在: {SOURCE_DATA_DIR}")

    # 清理输出目录
    _prepare_output_workspace(remove_raw=DO_TRANSFORM)

    # 获取文件夹列表
    folders = []
    for f in os.listdir(SOURCE_DATA_DIR):
        fp = os.path.join(SOURCE_DATA_DIR, f)
    ...
    if os.path.exists(raw_data_path_npz):
        ...
        raise FileNotFoundError(f"未找到原始数据文件: {raw_data_path_npz}")
...
if only_visualize:
    ...
    if os.path.exists(raw_data_path_npz):
        if DO_TRANSFORM:
            ...
            print(f"  ↳ 注意: DO_TRANSFORM=False，训练将使用此文件")
