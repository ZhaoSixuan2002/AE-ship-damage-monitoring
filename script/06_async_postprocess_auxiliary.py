"""
流程06辅助脚本：异步后处理（验证数据生成）
功能：使用 Abaqus Python 执行，打开指定 ODB，导出 CSV/NPY，可选删除 ODB
设计用于与下一次求解并行执行，支持三种验证数据类型（health/crack/corrosion）

用法（由主脚本调用）：
    abaqus python 06_async_postprocess_auxiliary.py -- <odb_path> <output_folder> <iteration_num>
...
# ========================================
if len(sys.argv) < 4:
    print("用法: 06_async_postprocess_auxiliary.py -- <odb_path> <output_folder> <iteration_num>")
    sys.exit(2)
...
if ENABLE_TIMING_LOG:
    try:
        ...
        # 读取已有 timings.csv（由06_single_run_auxiliary写入）
